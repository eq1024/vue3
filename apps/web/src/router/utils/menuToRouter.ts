 ...
 * @returns 处理后的路由配置
 */
export function menuDataToRouter(route: AppRouteRecord, parentPath = ''): AppRouteRecord {
 ...
 * @returns 构建后的完整路径
 */
function buildRoutePath(route: AppRouteRecord, parentPath: string): string {
  if (!route.path)
    return ''
 ...
 * @returns 处理后的子路由数组
 */
function processChildren(children: AppRouteRecord[], parentPath: string): AppRouteRecord[] {
 ...
 */
export function saveIframeRoutes(list: AppRouteRecord[]): void {
  if (list.length > 0) {
 ...
 * @returns iframe 路由列表
 */
export function getIframeRoutes(): AppRouteRecord[] {
  try {
