...
// 构建菜单类型标签
function buildMenuTypeTag(row: AppRouteRecord) {
...
// 构建菜单类型文本
function buildMenuTypeText(row: AppRouteRecord) {
  if (row.children && row.children.length > 0) {
    return '目录'
  }
  else if (row.meta?.link && row.meta?.isIframe) {
...
function showDialog(type: string, row: AppRouteRecord) {
...
const disableMenuType = computed(() => {
  ...
  if (isEdit.value && labelPosition.value === 'menu')
    return true
  // 顶部添加菜单按钮时锁定为菜单类型
  if (!isEdit.value && labelPosition.value === 'menu' && lockMenuType.value)
    return true
  return false
})
...
<template>
  <div class="menu-page art-full-height">
    ...
    <ElCard class="art-table-card" shadow="never">
      <!-- 表格头部 -->
      <ArtTableHeader v-model:columns="columnChecks" :show-zebra="false" @refresh="handleRefresh">
        <template #left>
          <!-- 按钮权限：后端控制模式，使用自定义指令 -->
          <ElButton v-auth="'add'" v-ripple @click="showModel('menu', null, true)">
            添加菜单
          </ElButton>
          ...
          <ElButton v-if="hasAuth('add')" v-ripple @click="showModel('menu', null, true)">
            添加菜单
          </ElButton>
        </template>
      </ArtTableHeader>
            ...
            <ElRow :gutter="20">
              ...
              <ElCol :span="12">
                <ElFormItem label="图标" prop="icon">
                  <ArtIconSelector v-model="form.icon" :icon-type="iconType" width="100%" />
                </ElFormItem>
              </ElCol>
            </ElRow>
