 ...
 * @returns 处理后的列属性
 */
function getColumnProps(col: ColumnOption<T>) {
 ...
 * @param expand 是否展开
 */
function processTreeRows(rows: T[], expand: boolean) {
  if (!tableRef.value)
    return
...
// ========== 事件处理 ==========
function handleRowClick(row: T, column: TableColumnCtx<T>, event: Event) {
  emit('row:click', row, column, event)
}

// 行双击事件
function handleRowDblclick(row: T, column: TableColumnCtx<T>, event: Event) {
  emit('row:dblclick', row, column, event)
}

// 行右键事件
function handleRowContextmenu(row: T, column: TableColumnCtx<T>, event: Event) {
...
// 行当前行事件
function handleCurrentRowChange(currentRow: T | null, oldCurrentRow: T | null) {
  emit('row:current-change', currentRow, oldCurrentRow)
}

function handleCellClick(row: T, column: TableColumnCtx<T>, cell: HTMLElement, event: Event) {
  emit('cell:click', row, column, cell, event)
}

// 行单元格双击事件
function handleCellDblclick(row: T, column: TableColumnCtx<T>, cell: HTMLElement, event: Event) {
  emit('cell:dblclick', row, column, cell, event)
}

// 行单元格鼠标进入事件
function handleCellMouseEnter(row: T, column: TableColumnCtx<T>, cell: HTMLElement, event: Event) {
  emit('cell:mouse-enter', row, column, cell, event)
}

// 行单元格鼠标离开事件
function handleCellMouseLeave(row: T, column: TableColumnCtx<T>, cell: HTMLElement, event: Event) {
  emit('cell:mouse-leave', row, column, cell, event)
}

// 表头点击事件
function handleHeaderClick(column: TableColumnCtx<T>, event: Event) {
  emit('header:click', column, event)
}

// 表头右键事件
function handleHeaderContextmenu(column: TableColumnCtx<T>, event: Event) {
...
// 过滤事件
function handleFilterChange(filterInfo: Record<string, any>) {
  emit('filter:change', filterInfo)
}

// 展开事件
function handleExpandChange(row: T, expandedRows: T[]) {
  emit('expand:change', row, expandedRows)
}

// 选择事件
function handleSelect(selection: T[], row: T) {
